/*
  # Database Performance Optimization and Schema Fixes

  1. Schema Standardization
    - Ensure consistent column naming for album art
    - Add missing columns if needed
    
  2. Performance Indexes
    - Add critical indexes for queue operations
    - Add indexes for song search and filtering
    - Add indexes for set list operations
    
  3. Database Functions
    - Optimized lock/unlock functions
    - Improved voting functions
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_priority 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_is_locked 
ON requests (is_locked) 
WHERE is_locked = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_is_played 
ON requests (is_played);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_title 
ON requests (title);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_title_artist 
ON requests (title, artist);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_created_at 
ON requests (created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_lookup 
ON requests (is_played, votes DESC, created_at DESC) 
WHERE is_played = false;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requests_title_not_played 
ON requests (title, is_played) 
WHERE is_played = false;

-- User votes indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_votes_lookup 
...
-- Requesters indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requesters_request_id 
ON requesters (request_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requesters_created_at 
ON requesters (created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_requesters_request_id_name 
...
-- Songs indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_songs_title_artist 
ON songs (title, artist);

-- Set lists indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_set_lists_is_active 
ON set_lists (is_active) 
WHERE is_active = true;

-- Set list songs indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_set_list_songs_set_list_id 
ON set_list_songs (set_list_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_set_list_songs_song_id 
ON set_list_songs (song_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_set_list_songs_both_ids 
